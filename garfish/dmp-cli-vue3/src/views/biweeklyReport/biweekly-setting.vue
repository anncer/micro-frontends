  ...
  >
    <el-tabs v-model="active" class="biweekly-tabs">
      ...
      <el-tab-pane label="权限配置" name="config" class="biweekly-set">
        <div
          style="margin-bottom: 20px; text-align: right; padding-right: 20px"
        >
        ...
        <ce-table :columns="settingColumns" :data="settingList" stripe>
          <el-table-column align="center" label="操作" width="140">
            <template #default="scope">
              <span class="biweekly-t-btn" @click="handleOpera(scope.row)"
                >编辑</span
              >
              <span
        ...
        >
          <div style="padding: 20px; padding-bottom: 0">
            ...
            <div
              style="text-align: center; margin-top: 40px; padding-bottom: 10px"
            >
              <ce-button @click="handleConfirm">确定</ce-button>
              <ce-button plain style="margin-left: 20px" @click="handleCancle">
                取消</ce-button
              >
            </div>
            <div></div>
...
const handleConfirm = () => {
  if ((selectionUser.value || editUserName) && selectionDept.value.length) {
    ...
    if (editUserName.value && editUserId.value) {
      ...
      const selectedUserArr = allUserList.value.filter(
        (it: any) => it.userId === _userId
      );
...
function doEditPermission(data: any) {
  setPermissionEdit(data).then((res) => {
    if (res.code === 200) {
      ElMessage({
        message: "操作成功",
        type: "success",
...
function getUsers() {
  getPermissionUser().then((res: any) => {
    if (res.code === 200) {
...
function getDepts() {
  getPermissionDept().then((res: any) => {
    if (res.data) {
...
const handleDelete = (row: any) => {
  ElMessageBox.confirm("删除后将不能恢复，确定删除吗?", "提示", {

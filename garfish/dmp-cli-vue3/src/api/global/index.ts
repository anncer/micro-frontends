...
import { IBaseRequest } from "./types.d";
...
// 获取查询条件下的所有人员不分页
export function getAllUsers(params?: unknown) {
...
// 获取查询条件下的所有人员分页
export function getPageUsers(page: number, size: number, params?: unknown) {
  return Api.post<IBaseRequest<unknown>>({
    url: GlobalApi.getUser,
    data: {
      enabledPage: true,
 ...
 * @param id
 */
export function downLoadAsStream(id: unknown) {
  return Api.get<IBaseRequest<unknown>>({
...
// 文件批量下载
export function downLoadFileMut(data: unknown) {
  return Api.post<IBaseRequest<unknown>>({
